...
public class CustomerDao {
    ...
    public PageBean<Customer> query(Customer customer,int pc,int pr){
        try
        {
            ...
            StringBuilder limitqSql = new StringBuilder(" limit ?,?");

            params.add((pc-1)*pr);
            params.add(pr);

            List<Customer> beanList = qr.query(sql.append(whereSql).append(limitqSql).toString(),new BeanListHandler<Customer>(Customer.class),params.toArray());
            pb.setBeanList(beanList);
